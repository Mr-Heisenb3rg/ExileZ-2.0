...
if (A2Buildings) then {_buildings = nearestObjects[_triggerPosition,["House"], SpawnRadius];}
else
...
if (Debug) then {diag_log format["ExileZ 2.0: Setting Primary Trigger	|	Position : %1	|	Spawn Radius : %2m	|	GroupSize : %3	|	Buildings : %4	|	Spawn Positions : %5	|	Near : %6 ",_triggerPosition,SpawnRadius,_groupSize,Count _buildings,_posCount,_nearestLocation];
};
}else{	
...
{if (isNull _group) then {nul = call InitGroup;
if (_newAct) then {for "_x" from 1 to _groupSize do{if (triggeractivated (_this select 0)) then{if (isNull _group) then {
nul = call InitGroup; sleep 1;};
nul = call Populate;
};sleep SpawnDelay;};
_newAct = false;
} else
...
_cnt = {alive _x} count units _group;
