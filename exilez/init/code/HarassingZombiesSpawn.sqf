...
if (Debug) then {diag_log format["ExileZ 2.0: No suitable spawn location found for a Harassing Zombie near %1 ",name (_this select 0)];};
} else { 
_zClass = HarassingGroup call GetZombie;
if (Debug) then {diag_log format["ExileZ 2.0: Spawning 1 Harassing Zombie  | Position : %1 | Class : %2  | For Player : %3 ",_zombieSpawnPosition,_zClass,name (_this select 0)];};
_zClass createUnit [
_zombieSpawnPosition,
_group, "
...
} else {                       //group is not empty or all dead
...
_x setdamage 1;
deleteVehicle _x;
if (Debug) then {diag_log format["ExileZ 2.0: Deleting 1 Harassing Zombie  | Position : %1 | Class : %2  | From Player : %3 ",Position _x,typeOf _x,name (_this select 0)];
};
};
